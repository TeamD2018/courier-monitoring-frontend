export const REQUEST_COURIERS_BY_BOX_FIELD = 'REQUEST_COURIERS_BY_BOX_FIELD';
export const REQUEST_COURIERS_BY_POLYGON = 'REQUEST_COURIERS_BY_POLYGON';
export const RECEIVE_COURIERS = 'RECEIVE_COURIERS';
export const RECEIVE_COURIERS_FAILED = 'RECEIVE_COURIERS_FAILED';

export const SHOW_COURIERS_LIST = 'SHOW_COURIERS_LIST';
export const HIDE_COURIERS_LIST = 'HIDE_COURIERS_LIST';

export const SHOW_COURIER_DETAILS = 'SHOW_COURIER_DETAILS';
export const HIDE_COURIER_DETAILS = 'HIDE_COURIER_DETAILS';
export const CLOSE_COURIER_DETAILS = 'CLOSE_COURIER_DETAILS';

export const PAN = 'PAN';
export const SET_ZOOM = 'SET_ZOOM';

export const RECEIVE_ACTIVE_COURIER = 'RECEIVE_ACTIVE_COURIER';
export const RECEIVE_ACTIVE_COURIER_FAIL = 'RECEIVE_ACTIVE_COURIER_FAIL';
export const REQUEST_ACTIVE_COURIER = 'REQUEST_ACTIVE_COURIER';
export const REQUEST_ACTIVE_COURIER_WITH_ONLY_ORDER = 'REQUEST_ACTIVE_COURIER_WITH_ONLY_ORDER';
export const RESET_ACTIVE_COURIER = 'RESET_ACTIVE_COURIER';

export const RESET_ERROR = 'RESET_ERROR';

export const SET_SHOW_ONLY_FREE_COURIERS_FLAG = 'SET_SHOW_ONLY_FREE_COURIERS_FLAG';
export const RESET_SHOW_ONLY_FREE_COURIERS_FLAG = 'RESET_SHOW_ONLY_FREE_COURIERS_FLAG';

export const REQUEST_POLYGON = 'REQUEST_POLYGON';
export const RECEIVE_POLYGON_FAILED = 'RECEIVE_POLYGON_FAILED';
export const SET_POLYGON = 'SET_POLYGON';
export const RESET_POLYGON = 'RESET_POLYGON';
